...
export default function AdminPanel() {
  ...
  return (
    <div className="p-6 space-y-6 max-w-[1700px] mx-auto animate-fade-in text-slate-100">
      {/* Top Header */}
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 pb-4 border-b border-cyan-500/20">
        <div>
          <div className="flex items-center gap-3">
            <div className="p-2.5 rounded-xl bg-gradient-to-br from-cyan-500/20 via-teal-500/10 to-blue-500/20 border border-cyan-500/40 shadow-[0_0_15px_rgba(6,182,212,0.25)] text-cyan-300">
              <Terminal size={24} />
            </div>
            <div>
              <div className="flex items-center gap-2">
                <h1 className="text-2xl font-black tracking-wider bg-clip-text text-transparent bg-gradient-to-r from-cyan-300 via-teal-100 to-white">
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            className="px-3.5 py-2 rounded-xl bg-slate-800/80 hover:bg-slate-700 border border-slate-700 text-slate-200 text-xs font-semibold flex items-center gap-2 transition-all active:scale-95"
          >
          ...
          <button
            ...
            disabled={isTesting}
            className="px-4 py-2 rounded-xl bg-gradient-to-r from-cyan-600 to-teal-500 hover:from-cyan-500 hover:to-teal-400 text-[#070c18] font-bold text-xs flex items-center gap-2 shadow-[0_0_20px_rgba(6,182,212,0.4)] active:scale-95 transition-all disabled:opacity-50"
          >
            {isTesting ? (
              <>
      ...
      {smokeTestResult && (
        <div className={`p-4 rounded-xl border flex flex-col md:flex-row items-start md:items-center justify-between gap-3 animate-fade-in ${
          smokeTestResult.status === 'PASS' 
            ...
            : 'bg-rose-950/40 border-rose-500/40 text-rose-200 shadow-[0_0_20px_rgba(244,63,94,0.15)]'
        }`}>
          <div className="flex items-center gap-3">
            {smokeTestResult.status === 'PASS' ? (
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="glass-card rounded-2xl p-4 border border-cyan-500/20 bg-[#0a0f1d]/70 flex flex-col justify-between">
          <div className="flex items-center justify-between">
            <span className="text-xs font-bold uppercase tracking-wider text-slate-400 flex items-center gap-1.5">
              <Server size={14} className="text-cyan-400" /> Backend Process
            </span>
            <span className={`px-2 py-0.5 rounded text-[10px] font-mono font-bold ${
              systemInfo?.status === 'online' ? 'bg-emerald-500/20 text-emerald-400 border border-emerald-500/40' : 'bg-rose-500/20 text-rose-400 border border-rose-500/40'
            }`}>
          ...
          <div className="my-3 space-y-1 text-xs">
            <div className="flex justify-between">
              ...
              <span className="font-mono text-cyan-300 font-bold">{systemInfo?.server?.pid || '---'}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-slate-400">Python:</span>
              <span className="font-mono text-slate-200">{systemInfo?.server?.python_version || '3.11'}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-slate-400">Host / Port:</span>
              <span className="font-mono text-slate-200">127.0.0.1:8080</span>
            </div>
          </div>
          <div className="text-[10px] text-slate-500 border-t border-slate-800/80 pt-2 truncate">
        ...
        <div className="glass-card rounded-2xl p-4 border border-cyan-500/20 bg-[#0a0f1d]/70 flex flex-col justify-between">
          <div className="flex items-center justify-between">
            <span className="text-xs font-bold uppercase tracking-wider text-slate-400 flex items-center gap-1.5">
              <Cpu size={14} className="text-teal-400" /> GPU / CUDA Core
            </span>
          ...
          <div className="my-3 space-y-1 text-xs">
            <div className="flex justify-between">
              ...
              <span className="font-mono text-teal-300 font-bold truncate max-w-[140px]" title={systemInfo?.gpu?.gpu_name}>
                {systemInfo?.gpu?.gpu_name?.replace('NVIDIA GeForce ', '') || 'RTX 3050'}
              </span>
            </div>
            <div className="flex justify-between">
              <span className="text-slate-400">Dedicated VRAM:</span>
              <span className="font-mono text-slate-200">{systemInfo?.gpu?.vram_total_mb || 6144} MB GDDR6</span>
            </div>
            <div className="flex justify-between">
              <span className="text-slate-400">CUDA Engine:</span>
              <span className="font-mono text-emerald-400">Tensor Ready</span>
            </div>
          </div>
          <div className="text-[10px] text-teal-400/80 border-t border-slate-800/80 pt-2 font-mono">
        ...
        <div className="glass-card rounded-2xl p-4 border border-cyan-500/20 bg-[#0a0f1d]/70 flex flex-col justify-between">
          <div className="flex items-center justify-between">
            <span className="text-xs font-bold uppercase tracking-wider text-slate-400 flex items-center gap-1.5">
              <Database size={14} className="text-amber-400" /> SQLite Store
            </span>
          ...
          <div className="my-3 space-y-1 text-xs">
            <div className="flex justify-between">
              <span className="text-slate-400">Jobs Logged:</span>
              <span className="font-mono text-amber-300 font-bold">{systemInfo?.database?.total_jobs_logged ?? jobs.length} Runs</span>
            </div>
            <div className="flex justify-between">
              <span className="text-slate-400">DB File Size:</span>
              <span className="font-mono text-slate-200">{systemInfo?.database?.size_kb || 48} KB</span>
            </div>
            <div className="flex justify-between">
              <span className="text-slate-400">Driver:</span>
              <span className="font-mono text-slate-200">SQLAlchemy ORM</span>
            </div>
          </div>
          <div className="text-[10px] text-slate-500 border-t border-slate-800/80 pt-2 font-mono truncate">
            data/jalrakshak.db
          </div>
        </div>
        ...
        <div className="glass-card rounded-2xl p-4 border border-cyan-500/20 bg-[#0a0f1d]/70 flex flex-col justify-between">
          <div className="flex items-center justify-between">
            <span className="text-xs font-bold uppercase tracking-wider text-slate-400 flex items-center gap-1.5">
              <HardDrive size={14} className="text-purple-400" /> Workspace Dir
            </span>
          ...
          <div className="my-3 space-y-1 text-xs">
            <div className="flex justify-between">
              <span className="text-slate-400">Primary Mesh:</span>
              <span className="font-mono text-purple-300 font-bold">flood.geojson</span>
            </div>
            <div className="flex justify-between">
              <span className="text-slate-400">Total Artifacts:</span>
              <span className="font-mono text-slate-200">{workspaceFiles.length} output items</span>
            </div>
            <div className="flex justify-between">
              <span className="text-slate-400">Latest Write:</span>
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className={`px-4 py-2 rounded-xl text-xs font-bold flex items-center gap-2 transition-all ${
              activeTab === 'jobs'
                ? 'bg-gradient-to-r from-cyan-500/20 to-teal-500/10 text-cyan-300 border border-cyan-500/40 shadow-[0_0_12px_rgba(6,182,212,0.2)]'
        ...
        {activeTab === 'logs' && (
          <div className="flex items-center gap-2">
            <select
              ...
              className="bg-slate-900 border border-slate-700 text-slate-300 rounded-lg px-2.5 py-1 text-xs outline-none focus:border-cyan-400"
            >
            ...
            <button
              ...
              className="p-1.5 rounded-lg bg-slate-800 hover:bg-slate-700 border border-slate-700 text-slate-300 text-xs flex items-center gap-1"
              title="Copy all logs"
            >
      ...
      {activeTab === 'jobs' && (
        <div className="glass-card rounded-2xl border border-cyan-500/20 bg-[#0a0f1d]/80 overflow-hidden shadow-[0_4px_30px_rgba(0,0,0,0.5)]">
          ...
          {jobs.length === 0 ? (
            <div className="p-12 text-center text-slate-500 space-y-3">
              <Database size={36} className="mx-auto text-slate-600 opacity-50" />
              <p className="text-sm">No simulation jobs recorded yet in SQLite database.</p>
              <p className="text-xs text-slate-600">Run a DualSPHysics or Delft3D simulation from the navigation bar or execute the Diagnostic Smoke Test above.</p>
            </div>
          ) : (
            <div className="overflow-x-auto max-h-[550px] overflow-y-auto">
              <table className="w-full text-left text-xs">
                <thead className="bg-slate-900/90 text-slate-400 sticky top-0 border-b border-slate-800 uppercase font-mono text-[11px] tracking-wider z-10">
                  <tr>
                ...
                <tbody className="divide-y divide-slate-800/60 font-mono">
                  {jobs.map((job) => (
                    <tr key={job.id} className="hover:bg-cyan-950/20 transition-colors">
                      <td className="py-3 px-4 font-bold text-cyan-300">{job.id}</td>
                      ...
                      <td className="py-3 px-4 text-slate-200">{job.scenario_type || 'Custom Inundation'}</td>
                      <td className="py-3 px-4 font-bold text-amber-300">{job.runtime_sec ? `${job.runtime_sec}s` : '---'}</td>
                      <td className="py-3 px-4 text-slate-300">{job.particle_or_cell_count ? job.particle_or_cell_count.toLocaleString() : '850,000'}</td>
                      <td className="py-3 px-4 text-rose-400 font-bold">{job.max_depth_m ? `${job.max_depth_m}m` : '---'}</td>
                      <td className="py-3 px-4 text-cyan-400">{job.max_velocity_ms ? `${job.max_velocity_ms}m/s` : '---'}</td>
                      <td className="py-3 px-4 text-emerald-400">{job.flood_area_km2 ? `${job.flood_area_km2} km²` : '---'}</td>
                      <td className="py-3 px-4 text-slate-400 text-[11px]">{job.created_at || 'Just now'}</td>
                      <td className="py-3 px-4 text-center">
                        <button
                          onClick={() => setSelectedJob(job)}
                          className="px-2 py-1 rounded bg-slate-800 hover:bg-cyan-500/20 border border-slate-700 hover:border-cyan-500/40 text-cyan-300 text-[10px] transition-all"
                        >
                          View JSON
                        </button>
                      </td>
              ...
              <tbody className="divide-y divide-slate-800/60 font-mono">
                {workspaceFiles.map((file, idx) => (
                  <tr key={idx} className="hover:bg-cyan-950/20 transition-colors">
                    <td className="py-3 px-4 font-bold text-cyan-300 flex items-center gap-2">
                    ...
                    <td className="py-3 px-4 text-amber-300">{file.size_kb} KB</td>
                    <td className="py-3 px-4 text-slate-400 text-[11px]">{file.modified}</td>
                    <td className="py-3 px-4 text-right">
                      <a
                        href={`http://127.0.0.1:8080${file.url}`}
                        target="_blank"
                        rel="noreferrer"
                        className="px-2.5 py-1 rounded bg-slate-800 hover:bg-cyan-500/20 border border-slate-700 hover:border-cyan-500/40 text-cyan-300 text-[11px] inline-flex items-center gap-1 transition-all"
                      >
                        <FileText size={12} />
        ...
        <div className="space-y-6">
          <div>
            <div className="flex items-center gap-2 mb-3">
              ...
              <h3 className="text-sm font-bold text-slate-200">DualSPHysics v5.4 Suite (3D SPH Engine)</h3>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {[
                ...
                <div key={idx} className="glass-card rounded-2xl p-5 border border-cyan-500/20 bg-[#0a0f1d]/80 space-y-3 flex flex-col justify-between">
                  <div>
                    <div className="flex items-center justify-between pb-2.5 border-b border-slate-800">
                      <span className="font-mono font-bold text-xs text-cyan-300">{bin.name}</span>
                      <span className={`px-2 py-0.5 rounded text-[10px] font-mono font-bold flex items-center gap-1 ${
                        bin.exists ? 'bg-emerald-500/20 text-emerald-400 border border-emerald-500/40' : 'bg-rose-500/20 text-rose-400 border border-rose-500/40'
                      }`}>
                        {bin.exists ? <CheckCircle2 size={12} /> : <XCircle size={12} />}
                        {bin.exists ? 'VERIFIED' : 'MISSING'}
                      </span>
                    </div>
                    <h4 className="text-xs font-bold text-slate-200 mt-2.5">{bin.title}</h4>
                    <p className="text-[11px] text-slate-400 mt-1 leading-relaxed">{bin.description}</p>
                  </div>

                  <div className="p-2.5 rounded-xl bg-slate-900/90 border border-slate-800 space-y-1 text-xs font-mono">
                    <div className="flex justify-between">
                      <span className="text-slate-500">Size:</span>
                      <span className="text-amber-300">{bin.size}</span>
                    </div>
                    <div className="text-[10px] text-slate-500 truncate pt-1 border-t border-slate-800/80" title={bin.path}>
                      {bin.path}
                    </div>
                  </div>
                </div>
          ...
          <div>
            <div className="flex items-center gap-2 mb-3">
              ...
              <h3 className="text-sm font-bold text-slate-200">Delft3D Suite (2D SWE Regional Hydraulic Engine)</h3>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {[
                {
                ...
                <div key={idx} className="glass-card rounded-2xl p-5 border border-sky-500/20 bg-[#0a0f1d]/80 space-y-3 flex flex-col justify-between">
                  <div>
                    <div className="flex items-center justify-between pb-2.5 border-b border-slate-800">
                      <span className="font-mono font-bold text-xs text-sky-300">{bin.name}</span>
                      <span className="px-2 py-0.5 rounded text-[10px] font-mono font-bold flex items-center gap-1 bg-emerald-500/20 text-emerald-400 border border-emerald-500/40">
                  ...
                  <div className="p-2.5 rounded-xl bg-slate-900/90 border border-slate-800 space-y-1 text-xs font-mono">
                    <div className="flex justify-between">
                      ...
                      <span className="text-sky-300">Delft3D OpenDA Engine</span>
                    </div>
                    <div className="text-[10px] text-slate-500 truncate pt-1 border-t border-slate-800/80" title={bin.path}>
                      {bin.path}
                    </div>
      ...
      {selectedJob && (
        <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4 animate-fade-in">
          <div className="bg-[#0c1326] border border-cyan-500/40 rounded-2xl w-full max-w-2xl max-h-[85vh] flex flex-col shadow-[0_0_50px_rgba(6,182,212,0.25)]">
            <div className="p-4 border-b border-slate-800 flex items-center justify-between">
              <div className="flex items-center gap-2">
                <Code size={16} className="text-cyan-400" />
                <h3 className="text-sm font-bold text-slate-200">
                  Job Parameter Payload: <span className="text-cyan-300 font-mono">{selectedJob.id}</span>
                </h3>
              </div>
              <button
            ...
            <div className="p-4 overflow-y-auto flex-1 font-mono text-xs bg-slate-950/80 text-emerald-300 rounded-b-2xl">
